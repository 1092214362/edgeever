...
test.describe("new memo synchronization", () => {
  test("derives the complete title when the first meaningful block is an H1", async ({ page }) => {
    const derivedTitle = `首个一级标题 ${Date.now()}`;
    ...
    try {
      ...
      await page.keyboard.insertText(derivedTitle);

      await expect(page.getByPlaceholder("无标题")).toHaveValue(derivedTitle);
      await expect.poll(async () => {
        const response = await page.request.get(`/api/v1/memos/${created.memo.id}`);
        if (!response.ok()) return null;
        const body = await response.json() as { memo: { title: string | null } };
        return body.memo.title;
      }).toBe(derivedTitle);
    } finally {
      await page.request.delete(`/api/v1/memos/${created.memo.id}`);
      await page.request.delete(`/api/v1/memos/${created.memo.id}?permanent=1`);
    }
  });

  test("does not derive the title from an H1 after body paragraphs", async ({ page }) => {
    const sectionHeading = `后置一级标题 ${Date.now()}`;
    const createResponsePromise = page.waitForResponse(
      (response) => response.request().method() === "POST" && new URL(response.url()).pathname === "/api/v1/memos",
    );
    ...
    try {
      ...
      await page.keyboard.insertText("第二段正文");
      await page.keyboard.press("Enter");
      await page.keyboard.type("# ");
      await page.keyboard.insertText(sectionHeading);
