...
export const enUS = {
  ...
  systemInfo: {
    ...
    updateAvailableDescription: "<paragraph>Version v{{version}} is available. Cloudflare and Docker instances <strong>update automatically each day by default; no manual action is required</strong>.</paragraph><paragraph><strong>Do not use Sync fork to update EdgeEver</strong>; if you click it by mistake and it reports conflicts, simply close the dialog.</paragraph><paragraph>To update immediately, Cloudflare users can run the Update deployed EdgeEver workflow, while Docker users can run ~/edgeever/update.sh.</paragraph><paragraph>The desktop app must be updated from the Releases page.</paragraph>",
