...
export const zhCN = {
  ...
  login: {
    ...
    instanceUnreachable: "无法连接到这个实例。请检查实例地址和网络，并确认实例服务正在运行。如果浏览器能打开但这里仍然失败，请检查 HTTPS 证书和跨域配置。",
    instanceApiNotFound: "这个地址没有找到 EdgeEver API。请填写实例根地址，不要填写管理页面或其他子页面地址。",
    invalidCredentials: "账号或密码错误，请检查后重试。",
    sessionExpired: "之前的登录状态已经失效，请重新输入账号和密码。",
    loginRateLimited: "登录失败次数过多，请稍等一会儿再试。",
    instanceServerError: "实例服务返回错误（HTTP {{status}}）。请稍后重试，或检查实例服务日志。",
    requestRejected: "实例拒绝了登录请求（HTTP {{status}}）。请检查实例地址和服务配置。",
    invalidInstanceResponse: "这个地址没有返回有效的 EdgeEver 响应，请确认它指向 EdgeEver 实例。",
    unexpectedError: "登录过程未能完成。请重启桌面客户端后重试；如果问题持续，请反馈下方诊断代码。",
