...
Docker 将 SQLite 与本地附件统一持久化到 `/data` 卷，也支持 S3 兼容附件存储。中国大陆用户可设置 `EDGE_EVER_IMAGE=ccr.ccs.tencentyun.com/edgeever/edgeever` 使用腾讯云公共镜像。镜像选择、HTTPS、Secret、NAS 权限、备份与升级说明请查看 [Docker 部署文档](docs/deploy-docker.zh-CN.md)。
