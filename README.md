...
5. **Build & Verify**: Keep Cloudflare's default deploy command (`npx wrangler deploy`) and start the first build. EdgeEver routes it through the same validated database migration and deployment pipeline automatically. Once complete, visit `/api/health` to verify a `200` response before logging in.
