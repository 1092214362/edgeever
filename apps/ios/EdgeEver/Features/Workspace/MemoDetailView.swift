...
struct RevisionsView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                ForEach(revisions) { rev in
                    VStack(alignment: .leading, spacing: 4) {
                        Text(env.preferences.t("修订 \(rev.revision)", en: "Revision \(rev.revision)"))
                            .font(.headline)
                        Text(MemoPreviewDate.format(rev.createdAt, locale: env.preferences.resolvedLocale))
                            .font(.caption)
                            .foregroundStyle(AppTheme.secondary)
                        Text(rev.contentText.prefix(120))
                            .font(.caption)
                            .foregroundStyle(AppTheme.secondary)
                            .lineLimit(2)
                        Button(env.preferences.t("恢复此版本", en: "Restore")) {
            ...
            .navigationTitle(env.preferences.t("修订历史", en: "Revisions"))
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(env.preferences.t("关闭", en: "Close")) { onRestored() }
                }
            }
            .task { await load() }
        }
    ...
    private func load() async {
        do {
            revisions = try await env.session.client.listMemoRevisions(memoId: memoId)
        } catch {
            self.error = error.localizedDescription
        }
    }
