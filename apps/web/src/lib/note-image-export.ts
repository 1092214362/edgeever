...
const resolveTheme = (background?: NoteImageBackground, theme?: NoteImageTheme): NoteImageTheme => {
...
export const buildNoteImageCardMarkup = ({
  ...
  const terminalBarHtml = isMono
    ? `<div class="edgeever-terminal-header">
        ...
        <span class="edgeever-terminal-name">${escapeHtml(notebook || "note.md")}</span>
      </div>`
    : "";
  ...
  if (hasMeta) {
    ...
    if (showTags && tags.length > 0) {
      for (const tag of tags) {
        parts.push(`<span class="edgeever-meta-pill edgeever-meta-tag">#${escapeHtml(tag)}</span>`);
      }
    }
    metaHtml = `<div class="edgeever-card-meta">${parts.join("")}</div>`;
  }

  const titleHtml = showTitle && title ? `<h1 class="edgeever-card-title">${escapeHtml(title)}</h1>` : "";
