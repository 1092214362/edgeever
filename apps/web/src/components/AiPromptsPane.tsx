...
import type { AiPromptTemplate } from "@edgeever/shared";
import {
  ChevronLeft,
  Eye,
  Loader2,
...
export const AiPromptsPane = ({ onClose }: { onClose: () => void }) => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editing) {
        return api.updateAiPrompt(editing.id, {
          name,
  ...
  const restoreMutation = useMutation({
    mutationFn: () => api.restoreDefaultAiPrompts(),
    onSuccess: async (result) => {
      ...
      queryClient.setQueryData(["ai-prompts"], result.prompts);
      await refresh();
    },
  });
          ...
          {editorOpen ? (
            ...
            >
              <div className="mb-4 flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <Button size="icon" variant="ghost" className="h-7 w-7 text-slate-400 hover:text-slate-600" type="button" onClick={cancelEditor}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
              <div className="grid gap-4">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button type="submit" size="sm" variant="solid" disabled={!canSubmit || saveMutation.isPending}>
